...
Then 
\begin{align*}
    Hx_{t} - Hx_{t+1} = -\eta g_t
\end{align*}

hence finding
$ x_{t+1}=
\operatorname{arg}\operatorname*{min}_{x\in\mathcal{X}}h(x) 
$
...
$
\begin{equation}
    \nabla_x h(x) = \eta g_t + H_t x - H_t x_t \\
\end{equation}
Setting $\nabla_x h(x)=0$ gives 
$$
x= x_t - \eta H^{-1}g_t
...
We can see that this update rule is similar to SGD or GD-like update rule.
