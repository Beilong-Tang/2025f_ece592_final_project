...
Following Adagrad \cite{duchi2011adagrad},
\begin{equation}
    B_{\psi}(x,y) = \psi(x) - \psi(y) - \langle \nabla \psi(y), x-y\rangle
\end{equation} denotes the Bregman divergence where $\psi$ is a strongly convex and differentiable function.

Here, we give a simplified proof of the convergence of Adagrad.

Adagrad's simplified upudate rule is
...
\begin{theorem}
The update rule in Equation~\ref{eq:adagrad_update} is equivalent to the update rule: 
\[
x_{t+1} = x_t - \eta \nabla f(x_t),
\]
